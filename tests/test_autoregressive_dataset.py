...
class TestAutoregressiveDataset(unittest.TestCase):
    def test_sequence_format(self):
        vocab = build_vocab()
        dataset = MathExprDataset(vocab, num_examples=5, depth=3, min_digits=1, max_digits=3)
        seq = dataset[0]
        ...
        self.assertTrue(any(op in scratchpad_part for op in '+-*/') or scratchpad_part.isdigit())
